...
def getactualsales():
	...
	reqList = []
	for row in sales:
		resultList = {'ID': str(row['id']),'Item': str(row['item']), 'Image': str(row['image']),'Price': str(row['oldrp']),'Sale': str(row['newrp'])}
		reqList.append(resultList)
		resultList = {}
	return jsonify({'Url': row['url'],'Date_end': row['enddate'],'Date_start': row['startdate']}, reqList)